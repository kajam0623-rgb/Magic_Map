...
- 결과 탭: 전체 요약, 전철역, 동/읍/면, 생성 키워드
- 키워드 전체 복사, 선택 키워드 복사
- 중복 병합 ON/OFF, suffix 포함/제거, 전철역/동/읍/면 필터
- 생성 키워드 탭에서 네이버 검색량 조회 결과를 포함한 엑셀 파일 저장
...
생성 키워드 탭에서 `선택 키워드 검색량 조회` 또는 `전체 키워드 검색량 조회`를 실행한 뒤, 현재 표에 표시된 검색키워드와 네이버 검색량 결과를 엑셀 파일로 저장할 수 있습니다.

```txt
keyword_volume_results.xls
```

엑셀 파일에는 검색키워드, 기본키워드, 전체검색, PC검색, 모바일검색, 모바일비중, 경쟁도, 추천 용도가 포함됩니다. 아직 조회하지 않은 키워드는 `미조회`, 네이버 API에 검색 데이터가 없는 키워드는 `데이터 없음`으로 구분합니다.
...
9. 생성 키워드 탭에서 `검색량 엑셀 저장`을 눌러 `keyword_volume_results.xls`가 내려받아지는지 확인합니다.
...
7. 생성 키워드 탭에서 검색량을 조회한 뒤 `검색량 엑셀 저장`으로 엑셀 파일을 내려받아 한글과 검색량이 정상 표시되는지 확인합니다.
